    ...
    def get_avg_delay(self) -> float:
        try:
            ...
            if 'connection' in res:
                ...
                relevant_connections = [conn for conn in connections if int(conn['departure']['time']) - current_time < 1800]
                return avg([int(conn['departure']['delay']) for conn in relevant_connections])
            else:
